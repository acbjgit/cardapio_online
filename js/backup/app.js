...
cardapio.metodos = {
    ...
    // Diminuir quantidade
    diminuirQuantidade: (id, obsAtual) => {
        let item = MEU_CARRINHO.find(e => e.id === id && e.observacao === obsAtual);
        
        if (item) {
            let qntdAtual = item.quantidade; // Obtém a quantidade do item

            // Verifica se a quantidade atual é maior que 0 antes de diminuir
            if (qntdAtual > 0) {
                item.quantidade -= 1; // Diminui a quantidade
                $("#qntd-" + id + "-" + obsAtual).text(item.quantidade); // Atualiza a interface
    ...
    // Aumentar quantidade
    aumentarQuantidade: (id, obsAtual) => {
        let item = MEU_CARRINHO.find(e => e.id === id && e.observacao === obsAtual);
    ...
    removerItenCarrinho: (id, obsAtual) => {
        // Filtra o carrinho para remover o item que tem o mesmo ID e a mesma observação
        MEU_CARRINHO = $.grep(MEU_CARRINHO, (e) => {
            return !(e.id === id && e.observacao === obsAtual);
        });
    ...
    atualizarCarrinho: (id, obsAtual, qntd) => {
        let objIndex = MEU_CARRINHO.findIndex((obj) => obj.id == id && obj.observacao === obsAtual);
        
        if (objIndex !== -1) { // Verifica se o item foi encontrado
            MEU_CARRINHO[objIndex].quantidade = qntd; // Atualiza a quantidade
        } else {
            console.error("Item não encontrado no carrinho para atualizar.");
        }
